...
class Usuario(db.Model):
    ...
    is_active = db.Column(db.Boolean, default=True, nullable=False) # Para inativar usuários

    def set_password(self, password): # Adicionado para consistência, se não tiver
        self.senha_hash = bcrypt.generate_password_hash(password).decode('utf-8')

    def check_password(self, password): # Adicionado para consistência, se não tiver
...
def buscar_pets_por_criterios_db(especie=None, porte=None, temperamento_keywords=[], energia=None, idade_texto_pref=None):
    ...
    pets_data = []
    for pet in results:
        personalidades_nomes = [p.personalidade.nome for p in pet.personalidades_list if p.personalidade] 
        pets_data.append({
            "id": pet.id,
            "nome": pet.nome,
            "especie": pet.especie,
...
def get_ong_protetor_contact(ong_protetor_id):
    try:
        ...
        if not ong_protetor or not ong_protetor.aprovado or not ong_protetor.is_active: 
...
def get_animal_details(animal_id):
    try:
        ...
        if not animal or not animal.is_active or animal.status_adocao != 'Disponível': 
...
def register_user():
    ...
    if nome_organizacao and cnpj_cpf:
        ...
        new_ong = OngProtetor(
            ...
            is_active=True # Ativo por padrão, mas pode ser inativado por admin
        )
    ...
    # Registro de Usuário Comum
        ...
        new_user = Usuario(
            ...
            telefone=telefone,
            endereco=endereco,
            is_active=True # Usuários comuns são ativos por padrão
        )
...
def login_user():
    ...
    if admin_user and admin_user.check_password(senha):
        if not admin_user.is_active: # Verifica se o admin está ativo
    ...
    # --- 2. Tentar login como Usuário Comum ---
    user = Usuario.query.filter_by(email=email).first()
    if user and bcrypt.check_password_hash(user.senha_hash, senha):
        if not user.is_active: # Verifica se o usuário está ativo
    ...
    # --- 3. Tentar login como ONG/Protetor (Seu código original + is_active) ---
    ...
    if ong_protetor and bcrypt.check_password_hash(ong_protetor.senha_hash, senha):
        if not ong_protetor.is_active: # Verifica se a ONG está ativa
...
def get_animals():
    try:
        # Filtra apenas animais ativos e disponíveis para a listagem pública
...
@jwt_required()
@ong_protetor_required() # Usa o decorator para proteger a rota
def create_animal():
    ...
    try:
        ...
        if user_role == 'ong_protetor':
            ...
            if not ong_protetor or not ong_protetor.aprovado or not ong_protetor.is_active: # Verifica is_active
                return jsonify({"message": "ONG/Protetor não encontrado, não aprovado ou inativo."}), 403
        elif user_role == 'admin': # Admin também pode cadastrar animais
        ...
        novo_animal = Animal(
            ...
            is_active=True # Novo animal é ativo por padrão
        )
    ...
    try:
        ...
        for animal in my_animals:
            ...
            animals_data.append({
                ...
                "ong_protetor_id": animal.ong_protetor_id,
                "personalidades": personalidades_nomes,
                "is_active": animal.is_active # Incluir status de ativo
            })
        return jsonify(animals_data), 200
    except Exception as e:
...
def get_user_profile():
    ...
    if request.method == 'PUT':
        ...
        if user_role == 'usuario':
            ...
            if not user.is_active: # Não permite editar perfil se inativo
            ...
            return jsonify({"message": "Perfil de usuário atualizado com sucesso!", "profile": {
                ...
                "role": "usuario",
                "is_active": user.is_active # Incluir status de ativo
            }}), 200
            ...
            if not ong.is_active: # Não permite editar perfil se inativo
                return jsonify({"message": "Sua conta está inativa e não pode ser editada."}), 403
            ...
            return jsonify({"message": "Perfil de ONG/Protetor atualizado com sucesso!", "profile": {
                ...
                "role": "ong_protetor",
                "is_active": ong.is_active # Incluir status de ativo
            }}), 200
        elif user_role == 'admin': # Admin também pode ter um perfil para edição
            admin_user = Admin.query.get(user_id)
            if not admin_user:
                return jsonify({"message": "Administrador não encontrado."}), 404
            if not admin_user.is_active: # Não permite editar perfil se inativo
                return jsonify({"message": "Sua conta está inativa e não pode ser editada."}), 403
    ...
    # Lógica para requisições GET (Obter perfil) 
        ...
        elif user_role == 'admin': # Obter perfil de admin
            admin_user = Admin.query.get(user_id)
            if admin_user:
                return jsonify({
                    "id": admin_user.id,
...
def update_animal(animal_id):
    ...
    animal.is_active = data.get('is_active', animal.is_active) # Admin pode mudar o status de ativo
...
def admin_get_all_pets():
    ...
    animals_data = []
    for animal in animals:
        # Verifica se ong_protetor existe antes de acessar nome_organizacao
